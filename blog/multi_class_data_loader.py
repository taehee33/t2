...
class MultiClassDataLoader(object):
    ...
    def define_flags(self):
        self.__flags.DEFINE_string("train_data_file", "./data/xxx.train", "Data source for the training data.")
        self.__flags.DEFINE_string("dev_data_file", "./data/xxx.dev", "Data source for the cross validation data.")
        self.__flags.DEFINE_string("class_data_file", "./data/xxx.cls", "Data source for the class list.")
    ...
    def prepare_data(self):
        ...
        max_doc_len = max([len(doc) in x_train])
        #max_doc_len_dev = max([len(doc.decode("utf-8")) for doc in x_dev])
        max_doc_len_dev = max([len(doc) in x_dev])
